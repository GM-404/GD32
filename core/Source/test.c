...
int main()
{
    ...
    const uint8_t *current_frame_bytes = &packed_frame_data[PACKED_FRAME_SIZE];
    ...
    if (1) { // Assume dismantle successful for this example, or use your actual call
        ...
        if (perform_1d_fft(parsed_radar_data, fft_1d_output_data) == 0) {
            ...
            if (perform_2d_fft(fft_1d_output_data, fft_2d_output_data) == 0) {
                ...
                //     .training_cells_range = 1,    // 距离维度训练单元 (单侧)
                //     .training_cells_doppler = 1,  // 多普勒维度训练单元 (单侧)
                //     .threshold_factor = 0.0001,     // 阈值因子 (需要根据实际数据调整)
